...
# X-arms: does the model see what it already did? Same tool suite, but prefixed with
# 20k tokens of simulated earlier turns (5x the forced SWA window) - the pi condition.
# label | server args | harness args | extra container env | fallback extra args
DENSE="$CORE;$KV;$TPL;--chat-template-kwargs;$KW_PROD;$THINK;$MTP"
ARMS=(
  "X1_override|$BASE|--tools-only --long-tokens 20000||"
  "X2_final|$DENSE;--temp;0.6;--top-p;0.8;--top-k;20;--min-p;0;--repeat-penalty;1.0|--tools-only --long-tokens 20000||"
  "X3_json|$DENSE;--chat-template-kwargs;$KW_JSON;$SAMP|--tools-only --long-tokens 20000||"
)
